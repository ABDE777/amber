...
export async function createPaymentSession(order, returnUrl) {
  ...
  // Always read the live admin price — never rely on the compiled constant
  ...
  const totalMad = Math.round(qty * basePriceMad);
  ...
  try {
    ...
    console.log("[YouCan Pay Tokenize Raw Response]:", JSON.stringify(data, null, 2));
    console.log("[YouCan Pay] res.status:", res.status, "| token type:", typeof data.token, "| token value:", data.token);
