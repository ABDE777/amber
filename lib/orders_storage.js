...
export async function saveOrderToCsv(order, source = "AI Assistant") {
  ...
  if (isServerless()) {
    ...
    if (ghData !== null) {
      // Append new row to current GitHub content
      ...
      let newContent;
      if (!hasHeader) {
        newContent = CSV_HEADER + currentContent + "\n" + row;
      } else {
...
/**
 * Updates order status — reads from GitHub, patches, writes back.
 */
export async function updateOrderStatus(orderId, newStatus) {
  try {
    ...
    if (isServerless()) {
      const result = await writeToGitHub(output, `Update order ${orderId} status to ${newStatus}`);
      if (result.ok) {
    ...
    console.log(`[Storage] Order ${orderId} updated to: ${newStatus}`);
    return { ok: true, orderId, status: newStatus };

  } catch (err) {
    console.error("[Storage Error] updateOrderStatus:", err);
    return { ok: false, error: err.message };
  }
}
