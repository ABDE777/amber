...
export async function saveOrderToCsv(order, source = "Order Form") {
  // Preferred backend: Supabase (when SUPABASE_URL + key are configured).
  if (isSupabaseConfigured()) {
    const result = await sbInsertOrder(order, source);
    if (result.ok) return result;
    console.error("[Storage] Supabase insert failed, falling back to CSV/GitHub:", result.error);
  }
...
export async function readOrdersCsvAsync() {
  // Preferred backend: Supabase (source of truth when configured).
  if (isSupabaseConfigured()) {
    const csv = await sbReadOrdersCsv();
    if (csv !== null) return csv;
    console.error("[Storage] Supabase read failed, falling back to CSV/GitHub");
  }
...
export async function updateOrderStatus(orderId, newStatus, lockedPrice = null) {
  // Preferred backend: Supabase (when configured).
  if (isSupabaseConfigured()) {
    const result = await sbUpdateStatus(orderId, newStatus, lockedPrice);
    if (result.ok || result.error === "order_not_found") return result;
    console.error("[Storage] Supabase update failed, falling back to CSV/GitHub:", result.error);
  }
...
/**
 * Deletes an order by id. Uses Supabase when configured; otherwise rewrites the
 * CSV (on GitHub when serverless, on disk locally) with the row removed.
 */
export async function deleteOrder(orderId) {
  // Preferred backend: Supabase (when configured).
  if (isSupabaseConfigured()) {
    const result = await sbDeleteOrder(orderId);
    if (result.ok || result.error === "order_not_found") return result;
    console.error("[Storage] Supabase delete failed, falling back to CSV/GitHub:", result.error);
  }
  ...
  try {
    ...
    for (let i = startIndex; i < lines.length; i++) {
      const vals = parseCsvLine(lines[i]);
      if (vals.length >= 1 && vals[0] === orderId) {
        removed = true;
        continue;
      }
      if (vals.some((v) => v.trim())) newRows.push(vals.map(escapeCsv).join(","));
    }

    if (!removed) return { ok: false, error: "order_not_found" };

    const output = headers.map(escapeCsv).join(",") + "\n" + newRows.join("\n") + (newRows.length ? "\n" : "");

    if (isServerless()) {
      const result = await writeToGitHub(output, `Delete order ${orderId}`);
      if (result.ok) {
        try { fs.writeFileSync(path.join("/tmp", "orders.csv"), output, "utf8"); } catch {}
        return { ok: true, orderId };
      }
